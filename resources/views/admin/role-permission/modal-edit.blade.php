{{-- resources/views/admin/role-permission/modal-edit.blade.php --}}

<!-- Modal Edit User - OPTIMIZED WITH BETTER SPACING -->
<div x-show="openEditUser" x-cloak x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div @click.outside="openEditUser = false" 
        ...
        <div class="overflow-y-auto flex-1 px-6 pt-6">
            ...
            <form method="POST" :action="`{{ route('admin.users.index') }}/${selectedUser.id}`" id="editUserForm"
                x-data="{
                    ...
                    branches: {{ json_encode($branches->map(fn($b) => ['id' => $b->id, 'name' => $b->name])) }},
                    ...
                    updateFieldsVisibility() {
                        ...
                        if (role) {
                            ...
                            this.showBranchEdit = role.name === 'Branch Coordinator' || role.name === 'Participant';
                        }
                        ...
                        if (this.editBranchId) {
                            const branch = this.branches.find(b => b.id == this.editBranchId);
                            if (branch) this.editBranchName = branch.name;
                        }
                    },
                    ...
                    selectBranch(branchId, branchName) {
                        ...
                        this.editBranchName = branchName;
                        this.branchDropdownOpen = false;
                    }
                }"
                x-init="
                    $watch('editRoleId', () => updateFieldsVisibility()); 
                    updateFieldsVisibility();
                    if (editRoleId) {
                        const role = roles.find(r => r.id == editRoleId);
                        if (role) editRoleName = role.name;
                    }
                    if (editBranchId) {
                        const branch = branches.find(b => b.id == editBranchId);
                        if (branch) editBranchName = branch.name;
                    }
                ">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-5 pb-6">
                    ...
                    <div class="relative z-30" id="editRoleDropdownContainer">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            Role <span class="text-red-500">*</span>
                        </label>
                        
                        <!-- Hidden Input -->
                        <input type="hidden" name="role_id" :value="editRoleId" required>
                        
                        <!-- Dropdown Button -->
                        <button type="button"
                                @click="roleDropdownOpen = !roleDropdownOpen; branchDropdownOpen = false; if(roleDropdownOpen) scrollToElement('editRoleDropdownContainer')"
                            ...
                            <span :class="editRoleName ? 'text-gray-900' : 'text-gray-400'" x-text="editRoleName || 'Pilih Role'"></span>
                            ...
                            <div class="overflow-y-auto max-h-full">
                                <template x-for="role in roles" :key="role.id">
                                    <button type="button"
                                            ...
                                            :class="editRoleId == role.id ? 'bg-[#10AF13]/20' : ''">
                                        <span x-text="role.name" class="text-sm font-medium" :class="editRoleId == role.id ? 'text-[#10AF13]' : 'text-gray-700'"></span>
                                        <svg x-show="editRoleId == role.id" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-[#10AF13]">
                    ...
                    <div x-show="showBranchEdit" x-transition class="relative z-20" id="editBranchDropdownContainer">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            Cabang <span class="text-red-500" x-show="showBranchEdit">*</span>
                        </label>
                        
                        <!-- Hidden Input -->
                        <input type="hidden" name="branch_id" :value="editBranchId" :required="showBranchEdit">
                        
                        <!-- Dropdown Button -->
                        <button type="button"
                                @click="branchDropdownOpen = !branchDropdownOpen; roleDropdownOpen = false; if(branchDropdownOpen) scrollToElement('editBranchDropdownContainer')"
                                class="w-full px-4 py-3 bg-white border-2 rounded-lg focus:outline-none transition text-left flex items-center justify-between"
                                :class="branchDropdownOpen ? 'border-[#10AF13] ring-2 ring-[#10AF13]/30' : 'border-gray-300'">
                            <span :class="editBranchName ? 'text-gray-900' : 'text-gray-400'" x-text="editBranchName || 'Pilih Cabang'"></span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" 
                                 class="transition-transform duration-200"
                            ...
                            <div class="overflow-y-auto max-h-full">
                                <template x-for="branch in branches" :key="branch.id">
                                    <button type="button"
                                            ...
                                            class="w-full px-4 py-3 text-left hover:bg-[#10AF13]/10 transition flex items-center justify-between border-b border-gray-100 last:border-b-0"
                                            :class="editBranchId == branch.id ? 'bg-[#10AF13]/20' : ''">
                                        <span x-text="branch.name" class="text-sm font-medium" :class="editBranchId == branch.id ? 'text-[#10AF13]' : 'text-gray-700'"></span>
                                        <svg x-show="editBranchId == branch.id" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-[#10AF13]">
                                            <path d="M5 12l5 5l10 -10" />
                                        </svg>
                                    </button>
                                </template>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            Password Baru <span class="text-gray-400"></span>
                        </label>
                        <div class="relative">
