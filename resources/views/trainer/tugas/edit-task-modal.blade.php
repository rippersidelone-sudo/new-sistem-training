{{-- resources/views/trainer/tugas/edit-task-modal.blade.php --}}
<div x-show="openEditModal" x-cloak x-transition
    ...
    <div @click.outside="openEditModal = false"
        ...
        <div class="bg-[#10AF13] px-6 py-5 text-white flex items-center justify-between flex-shrink-0">
            ...
            <button @click="openEditModal = false" class="text-white hover:text-gray-200 transition">
        ...
        <div class="p-6 overflow-y-auto flex-1">
            <form :action="`{{ url('trainer/tasks') }}/${currentTask?.id}`" method="POST"
                  x-data="{
                      editDate: '',
                      editTime: '23:59',
                      get deadlineCombined() {
                          return (this.editDate || '') + 'T' + (this.editTime || '23:59');
                      }
                  }"
                  x-init="
                      $watch('$root.currentTask', val => {
                          if (val?.deadline) {
                              editDate = val.deadline.split('T')[0] ?? '';
                              editTime = val.deadline.split('T')[1]?.slice(0, 5) ?? '23:59';
                          }
                      })
                  "
                ...
                <div class="space-y-5">
                    ...
                    <div x-data="{
                            ...
                            init() {
                                this.$watch('$root.currentTask', val => {
                                    if (val?.batch_id) {
                                        const found = this.options.find(o => o.value === String(val.batch_id));
                                        this.selectedValue = String(val.batch_id);
                                        this.selectedLabel = found ? found.label : 'Pilih Batch';
                                    }
                                });
                            },
                            select(value, label) {
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="5" x-model="currentTask.description"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input type="time" x-model="editTime"
                                       class="w-full px-4 py-3 bg-white border border-gray-300 rounded-lg focus:outline-none focus:border-[#10AF13] focus:ring-1 focus:ring-[#10AF13] transition"
                                       required>
                            </div>
                        </div>
